...
class TestPostUpdateView:
    ...
    @pytest.mark.parametrize(
        "user,is_visible", [(lambda: UserFactory(), False), (lambda: UserFactory(is_staff=True), True)]
    )
    def test_approved_field_visibility(self, client, db, user, is_visible):
        user = user()
        topic = TopicFactory(with_post=True, answered=True, forum=ForumFactory(with_public_perms=True))
        assign_perm("can_edit_posts", user, topic.forum)
        client.force_login(user)
        ...
        assert response.status_code == 200
        assert ("div_id_approved" in response.content.decode()) == is_visible
