...
class DSPWorkCapacity(models.TextChoices):
...
class DSPLanguageSkills(models.TextChoices):
...
class DSPHousing(models.TextChoices):
...
class DSPRightsAccess(models.TextChoices):
...
class DSPMobility(models.TextChoices):
    ...
    LEVEL3 = "3", "Possède un véhicule et permis de conduire conformes et covoitureur possible"


class DSPResources(models.TextChoices):
...
class DSPJudicial(models.TextChoices):
    LEVEL_0 = "0", "Exécution de peine en cours "
    LEVEL_1 = "1", "Suivi SPIP/PJJ après exécution de peine"
    LEVEL_2 = "2", "Aucun suivi en cours"


class DSPAvailability(models.TextChoices):
